...
module.exports = (srv) => {
  srv.on("CREATE", "BPA", async (req) => {
    ...
    try {
      await INSERT.into('db.BPA').entries(req.data);
      ...
      try {
        ...
        const workflow = await cds.connect.to('workflow-api');
        await workflow.tx(req).post("/workflow-instances", wfPayload);
        console.log("✅ Workflow started via destination");
        console.log(JSON.stringify(process.env.destinations, null, 2));
      } catch (workflowError) {
        console.log(JSON.stringify(process.env.destinations, null, 2));
        console.error("⚠️ Workflow not started:", workflowError.message);
      }
...
// CODES FOR IMPLEMENTATION OF AXIOS MODULE TO CALL THE WORKFLOW
